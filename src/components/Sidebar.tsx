  ...
  return (
    <aside className="w-full md:w-60 shrink-0 h-auto md:h-[calc(100vh-56px)] bg-black/20 border-r border-white/10 p-3 overflow-y-auto">
      <nav className="space-y-1 text-sm">
        ...
        {hasRole('admin') && (
          <NavLink to="/usuarios" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}>
            <UsersIcon className="w-4 h-4" /> Usuarios
          </NavLink>
        )}
        <NavLink to="/clientes" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}>
          <UserGroupIcon className="w-4 h-4" /> Clientes
        </NavLink>
        <NavLink to="/catalogo" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}>
          <Squares2X2Icon className="w-4 h-4" /> Catálogo
        </NavLink>
        <NavLink to="/pesajes" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}> 
          <ScaleIcon className="w-4 h-4" /> Pesajes
        </NavLink>
        <NavLink to="/vehiculos" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}>
          <TruckIcon className="w-4 h-4" /> Vehículos
        </NavLink>
        {/* Oculto por solicitud: Gestiones */}
        {false && (
          <NavLink to="/gestiones" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}>
            <Squares2X2Icon className="w-4 h-4" /> Gestiones
          </NavLink>
        )}
        {/* Oculto por solicitud: Tolerancias */}
        {false && (
          <NavLink to="/tolerancias" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}>
            <TagIcon className="w-4 h-4" /> Tolerancias
          </NavLink>
        )}
        <NavLink to="/productos" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}>
          <CubeIcon className="w-4 h-4" /> Volumen
        </NavLink>
        {/* Oculto por solicitud: Re‑pesos */}
        {false && (
          <NavLink to="/repesos" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}>
            <ScaleIcon className="w-4 h-4" /> Re‑pesos
          </NavLink>
        )}
        
        <NavLink to="/reportes" className={({isActive})=>`${linkBase} ${isActive?activeCls:''}`}>
